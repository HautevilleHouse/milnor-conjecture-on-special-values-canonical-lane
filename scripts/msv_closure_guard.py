#!/usr/bin/env python3
"""Canonical-lane closure guard for the milnor conjecture on special values workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['MSV_G1', 'MSV_G2', 'MSV_G3', 'MSV_G4', 'MSV_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['MSV_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['MSV_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['MSV_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['MSV_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['MSV_G1'], 'G2': gates['MSV_G2'], 'G3': gates['MSV_G3'], 'G4': gates['MSV_G4'], 'G5': gates['MSV_G5'], 'G6': 'NA', 'GCoh': gates['MSV_G6'], 'GM': gates['MSV_GM']}
    normalized_blockers = {'G1': blockers['MSV_G1'], 'G2': blockers['MSV_G2'], 'G3': blockers['MSV_G3'], 'G4': blockers['MSV_G4'], 'G5': blockers['MSV_G5'], 'G6': [], 'GCoh': blockers['MSV_G6'], 'GM': blockers['MSV_GM']}
    report = {'meta': {'computed_at_utc': dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(), 'framework': 'milnor_conjecture_on_special_values'}, 'schema': {'normalized_gate_keys': ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'GCoh', 'GM'], 'g6_policy': 'NA means this framework has no standalone pre-coherence G6 gate'}, 'lane': {'canonical_theorem_lane': 'manifold_constrained', 'active_lane': 'manifold_constrained'}, 'inputs': {**primary_values, 'eps_coh': eps_coh, 'sigma_star_can': sigma_star}, 'derived': {'strict_margin': margin, 'strict_coh_zero': bool(strict_coh_zero)}, 'gates': gates, 'blockers': blockers, 'normalized': {'gates': normalized_gates, 'blockers': normalized_blockers}}
